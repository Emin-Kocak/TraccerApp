...
### 27. Gece yarısı gün-sızıntısı düzeltmesi (2026-07-14)
**Bug**: Kullanıcı ayın 12'sinde telefonu 4s / Instagram'ı 2s kullandı; gece yarısı geçip 13'üne girince Dashboard hâlâ 12'nin kullanımlarını gösteriyordu. Kök neden **iki katmanlı**:
- **Veri sızıntısı (birincil)**: `AppAccessibilityService.dailyUsageMs` in-memory sayacı gece yarısı **hiç sıfırlanmıyordu**. Servis (erişilebilirlik servisi süreç olarak gece boyunca yaşar) 12'nin toplamını biriktirmeye devam edip `flushToDBSuspend` içinde sabit `getTodayStartMs()` gününe yazıyordu → 12'nin 2 saati 13'ün `date` kovasına sızıyordu. `DailySyncWorker` reconcile'ı `maxOf(dbMs, sysMs)` kullandığı için sızan yüksek değeri **koruyordu**.
- **Bayat gösterim (ikincil)**: UI'da "bugün" değeri `remember {}` ile bir kez hesaplanıp yaşam döngüsüne bağlanmadığından, gece yarısını aşan uzun ömürlü composition dünün gününde donuyordu.

**Düzeltme**:
- Servis: yeni `usageDayStart` alanı birikimin ait olduğu günü tutar. `flushToDBSuspend` artık `date = bucketDay` (usageDayStart) yazıyor; `bucketDay != today` olunca (gece yarısı) önceki gün yazıldıktan sonra `seedDailyUsageFromDB(today)` ile sayaç sıfırlanıp yeni gün DB/OS'ten yeniden tohumlanıyor. Sınır: son gece-yarısı-öncesi flush ile ilk sonrası flush arası ≤10dk (`FLUSH_INTERVAL_MS`) kullanım önceki güne yazılabilir — `DailySyncWorker` OS reconcile ile düzelir, 2 saatlik sızıntı ortadan kalkar.
- UI: ortak `rememberTodayStart()` composable'ı (`ui/screens/RememberTodayStart.kt`) — `startOfDay(now)`'u her `RESUMED`'da yeniden hesaplar. Dashboard (MainScreen), ReportsScreen, UsageAnalysisScreen üçü de kullanıyor. Dashboard'daki `refreshUsageStats`/`fetchTodayHourBlocks` `LaunchedEffect`'leri `todayStart`'a bağlandı (gün dönünce düzeltilmiş sorgunun boş kovaya düşmemesi için). Bilinen sınır: uygulama gece yarısı boyunca kesintisiz ön planda kalırsa (RESUME tetiklenmez) gün, ekran kapanıp açılana dek güncellenmez — ekran süresi uygulaması için ihmal edilebilir.

**Durum**: derleme + tüm unit testler BUILD SUCCESSFUL. **Rollover dalının gerçekten tetiklendiği cihazda doğrulanmadı** (compile bunu gösteremez). Saat-ileri-alma testi: (a) servis açık + Instagram'ı birkaç dk kullan (dailyUsageMs dolsun), (b) `adb shell` cihaz saatini gece yarısını geçecek şekilde ileri al, (c) kilit-aç ile flush tetikle (ACTION_SCREEN_OFF yolu), (d) doğrula: yeni günde Instagram ~0 (dünün toplamı değil), dünün kovası toplamını koruyor.

### 28. Ekran-kapalı süre şişmesi + iki küçük veri düzeltmesi (2026-07-16)
Genel yapı denetiminde bulunan 3 sorun tek oturumda düzeltildi:

1. **KRİTİK — ekran kapalıyken foreground oturumu hiç kapanmıyordu** (`AppAccessibilityService.screenReceiver`): `ACTION_SCREEN_OFF` commit yapıyor ama `currentPackage`/`sessionStartMs`'i sıfırlamıyordu. Kilit ekranı (`com.android.systemui`) `shouldIgnorePackage` ile yutulduğu için paket hiç değişmiyor, 10dk'lık `startPeriodicFlush` döngüsü **kilitli geçen her aralığı son foreground app'e kullanım olarak** eklemeye devam ediyordu (gece boyu +saatler). `DailySyncWorker`'ın `maxOf` reconcile'ı şişkin değeri düşüremediği için (madde 27'deki asimetriyle aynı) hata kalıcılaşıyordu; 30sn'lik in-app limit kontrolü de kilitliyken sahte `liveElapsed` ile blok tetikleyebiliyordu. **Fix**: SCREEN_OFF'ta `commitReelSession()` + `commitCurrentSession()` sonrası `currentPackage = null; sessionStartMs = 0L`. Kilit açılınca app'in window-state event'i oturumu yeniden başlatır; event gelmezse eksik süreyi OS reconcile tamamlar — hata artık düzeltilebilir yönde (undercount), düzeltilemez yönde (overcount) değil. Reel bütçe sayacı da artık kilitli süreyi reel izleme saymıyor.
2. **`UsageStatsUtils.fetchAndSaveUsageStats` queryUsageStats fallback'i dünün bucket'ını bugüne yazabiliyordu**: sorgu dünden başladığı için gece yarısını aşan oturumlarda dünün `INTERVAL_DAILY` toplamı (`lastTimeUsed` bugüne sarkınca) bugünün kaydı olarak girebiliyordu. **Fix**: fallback değeri `minOf(duration, now - startOfDay)` ile sınırlandı — bugünkü kullanım gece yarısından beri geçen süreyi aşamaz.
3. **Aynı fonksiyonun DB yazımı düz REPLACE idi, `DailySyncWorker`'ın `maxOf` politikasıyla çelişiyordu**: Dashboard'a her girişte OS değeri servisin (daha yüksek olabilen) birikimini eziyor, 10dk sonra servis flush'ı geri yazıyor → toplamlar iki kaynak arasında zıplıyordu. **Fix**: yazmadan önce günün mevcut satırları okunup `maxOf` reconcile uygulanıyor (worker ile aynı kural), dönüş değeri de reconcile edilmiş liste.

Aynı oturumda iki küçük ek düzeltme: **(a)** `MainActivity.isAccessibilityServiceEnabled` düz string karşılaştırması yerine `ComponentName.unflattenFromString` kullanıyor — ayar servisi kısa formda (`paket/.service.Sınıf`) sakladığında servis açıkken bile "izin gerekli" uyarısı görünüyordu; **(b)** Analiz `TrendCard`'ı ±%0.5 bandındaki (nötr) değişimlerde aşağı ok yerine düz çizgi (`Remove`) ikonu gösteriyor.

Derleme + tüm unit testler BUILD SUCCESSFUL. **Cihaz doğrulaması kullanıcıda**: bir uygulama açıkken telefonu kilitle, 30+ dk sonra aç — o uygulamanın süresi kilitli süre kadar artmamalı; kilit açılınca takip normal devam etmeli (Dashboard'da süre ilerlemeli).

### 29. Oturum bazlı kullanım izni — "Girişte Süre Sor" (2026-07-17)
Uygulama başına üçüncü engelleme modu: mod açık uygulamaya girişte tam ekran "Bu oturumda ne kadar kullanacaksın?" sorusu; seçilen dakika o oturumun bütçesi, dolunca mevcut blok overlay'i. Spec: `docs/superpowers/specs/2026-07-17-session-prompt-design.md`, plan: `docs/superpowers/plans/2026-07-17-session-prompt.md`.

- **Veri**: `AppLimit.isSessionPromptEnabled` kolonu, additive `Migration(3,4)` (DB v4, veri kaybı yok). `getActiveLimits` sorgusu `OR isSessionPromptEnabled = 1` içerir — servisin `limitsCache`'i bu uygulamaları da görür. Oturum durumu (`sessionRemainingMs`/`sessionLastExitMs`) **yalnızca in-memory** (reel bütçesi deseni, madde 26) — servis restart'ında kaybolur, yeniden sorar.
- **Kurallar**: 5 dk grace penceresi (kısa çıkış/kilit oturumu bozmaz, `SESSION_GRACE_MS`); bütçe yalnızca uygulama öndeyken düşer (soru ekranında geçen süre sayılmaz); süre dolunca `triggerBlock("Oturum süresi doldu")` + state temizlenir → yeni oturum ancak bilinçli yeniden girişle. Günlük limit bağımsız ve önceliklidir: `checkAndBlockIfNeeded` artık **Boolean döndürür**, blokladıysa soru sorulmaz.
- **Hassasiyet**: "Başla" anında kalan süreye zamanlanmış tek atımlık `sessionExpiryJob` + 30sn'lik in-app tick emniyet ağı (`checkActiveSessionExpiry`).
- **Overlay**: `showSessionPromptOverlay` — blok overlay'iyle aynı pencere altyapısı, `blockedOverlayPackage` set edilir (tek-overlay değişmezi: soru görünürken reel taraması/tekrar-blok susar). Preset chip'ler (5/10/15/30/60) + stepper (5–180 dk, klavye yok — `FLAG_NOT_FOCUSABLE` korunur), "Başla" ve "Kullanmayacağım" (Home). Bilinen küçük boşluk: `removeOverlayInternal` `currentPackage`'ı null'lar; "Başla" sonrası ilk window-state event'ine kadar kullanım sayacı boşta kalır (OS reconcile tamamlar), soru tekrar gelmez (`activeSessionPackage` kontrolü).
- **Saf mantık** `service/sessionprompt/SessionPromptLogic.kt`'de (`shouldPromptForSession`/`deductSessionBudget`/`clampSessionMinutes`), JUnit testleri `SessionPromptLogicTest` (9 test, PASS).
- **UI**: Limitler bottom sheet'inde "Girişte Süre Sor" switch'i; `ActiveLimitRow`'da mavi kum saati + "Oturum" rozeti; "Aktif Limitler" filtresi bu modu da kapsar.

Derleme + tüm unit testler BUILD SUCCESSFUL. **Cihaz doğrulaması kullanıcıda**: (1) mod açık uygulamada giriş → soru; (2) 5dk seç → dolunca blok; (3) <5dk çıkış-dönüş → soru yok, kalan devam; (4) >5dk sonra dönüş → yeni soru; (5) "Kullanmayacağım" → ana ekran; (6) günlük limit doluysa soru yerine direkt blok; (7) AppLimit/zamanlama/reel-blocker regresyonu yok.

### 30. Emülatör doğrulama turu: madde 28 kanıtlandı + iki yeni bug bulundu (2026-07-23)
Pixel_6 AVD'de (Android 16/API 36, GMS/Play imajı) madde 27/28/29'un kalan cihaz testleri tamamlandı, yol boyunca session-prompt'tan bağımsız iki gerçek bug bulunup düzeltildi.

**Madde 28 kanıtlandı**: Clock uygulamasını foreground'da bırakıp `KEYCODE_SLEEP` ile kilitledim; DB'deki `usage_logs.durationMs` 90 saniye kilitli bekleme boyunca **tam sabit kaldı** (önceki fix'ten önce bu süre boyunca artmaya devam ederdi). Kilidi açınca `▶ Session started` logu yeniden düştü — takip doğru şekilde devam etti.

**Yeni Bug A — `AppIconUtils.getLauncherApps` 5 dakikalık cache'i güvenilmezdi**: `pm.queryIntentActivities(LAUNCHER)` bu ortamda ara sıra eksik sonuç dönüyordu (bir çağrıda 58 gerçek launcher paketinden yalnızca 18'i). Kanıt: `BlockingSettingsScreen.kt`'deki `getInstalledApps` **aynı queryIntentActivities çağrısını cache'siz** yapıyor ve her seferinde tam sonuç veriyordu — fark cache'in kötü bir sonucu 5 dakika boyunca dondurmasıydı. Etki: Takip sekmesi, Zaman Raporu, Saatlik Detay ve Dashboard'un iki saatlik grafiği (`fetchTodayHourBlocks`/`UsageStatsUtils` → `AppIconUtils.shouldTrack`) gerçek kullanımın büyük kısmını "Kullanım yok" gösteriyordu — servisin kendi `shouldIgnorePackage` (blacklist) tabanlı DB yazımı etkilenmiyordu, o yüzden Dashboard'un DB-tabanlı bölümleri (hero kart, kategori çemberi, uygulama listesi) doğru kalıyordu; sorun yalnızca whitelist tabanlı ikinci pipeline'daydı (madde 12/14'teki "iki ayrı pipeline" ayrımı). **Fix**: `CACHE_TTL_MS` 5 dakikadan **10 saniyeye** düşürüldü — cache'i tamamen kaldırmak yerine (queryEvents döngüsü içindeki tekrarlı çağrılarda performans faydası korunsun diye), kötü bir sonucun ömrünü pratik olarak sıfıra indirdi. Cihazda doğrulandı: fix sonrası Takip/Zaman Raporu ekranları önceden görünmeyen kullanım bloklarını (Gboard, ek Clock oturumları) gösterdi.
**Bilinen sınır**: `queryIntentActivities`'in NEDEN ara sıra eksik döndüğü tam olarak teşhis edilemedi (muhtemelen GMS/Play Protect paket-görünürlüğü taramasının arka planda geçici bir gecikmesi) — kısa TTL semptomu maskeliyor, kök nedeni gidermiyor. Eğer ileride yine "Kullanım yok" şikayeti gelirse, whitelist yaklaşımını tamamen terk edip `AppAccessibilityService.shouldIgnorePackage`'daki gibi blacklist'e geçmek daha kalıcı bir çözüm olur.

**Yeni Bug B — sistem/fiziksel geri tuşu push ekranlarda hiç ele alınmıyordu**: `MainScreen.kt`'nin `currentScreen` state'iyle yönetilen Report/Detail/Settings ekranları yalnızca kendi `TopAppBar` geri okuna bağlıydı; sistem geri tuşuna basınca (`OnBackPressedCallback` kayıtlı olmadığı için) uygulama bir üst ekrana dönmek yerine **direkt arka plana düşüyordu/kapanıyordu**. Kullanıcıların çoğu ekrandaki ok yerine fiziksel/gesture geri tuşunu kullanır — bu davranış üst üste birkaç kez geri basınca uygulamadan tamamen çıkma hissi yaratıyordu. **Fix**: `MainScreen`'e `androidx.activity.compose.BackHandler` eklendi (`enabled = currentScreen != Screen.Main`), ekran-üstü ok'larla aynı geçişi yapıyor (Detail→Report, Report/Settings→Main); `Screen.Main`'deyken devre dışı, sistemin varsayılan davranışı (arka plana at) korunuyor. Cihazda doğrulandı: Zaman Raporu ve Ayarlar ekranlarından sistem geri tuşu artık Dashboard'a dönüyor.

Derleme + tüm unit testler BUILD SUCCESSFUL (fix'lerden sonra). Emülatörde tam tarama: Dashboard, Raporlar, Takip, Limitler, Analiz (Günlük/Haftalık/Aylık), Zaman Raporu, Saatlik Detay, Ayarlar (hedef/gizli-app/tema) — hepsi crash'siz, session-prompt madde 29'daki kalan "%90" test de bu turda tamamlandı (bkz. eski `docs/2026-07-17-test-devam-notu.md`, artık kapandı).

### 31. Reels/Shorts tespiti view-id → contentDescription + "Ana Sayfaya Dön" butonu (2026-07-23)
Gerçek cihazda (Samsung, fiziksel) yapılan `adb logcat` teşhisiyle madde 26'daki reel-blocker'ın **hiç çalışmadığı** bulundu ve yeniden yazıldı.

**Kök neden — view-id yaklaşımı geçersiz**: Instagram ve YouTube Shorts, view'lara accessibility id atamıyor (Litho/benzeri render — cihazda `viewIdResourceName` sayısı istisnasız 0). Madde 26'nın aradığı `clips_viewer_container` / `reel_player_page_container` gibi id'ler gerçekte hiç yok. **Fix**: tespit, oynatıcının `contentDescription`'ına geçirildi:
- Instagram: `"Reels videosu. Oynatmak"` — sadece `"Reels videosu"` YETMEZ; ana feed'deki "X bu Reels videosunu beğendi" ve "Önerildi ...'dan Reels videosu" kartları bu dizeyi içeriyor ve ana sayfada yanlış blok tetikliyordu. `". Oynatmak"` eki gerçek oynatıcıya özgü (cihaz teşhisiyle doğrulandı).
- YouTube: `"Bu sesi kullanan daha fazla video izleyin"` (Shorts remix/ses özelliği — normal izleme ekranında yok).
- `NodeSearch.kt`: `containsViewId` kaldırıldı, ortak `containsContentDescription` kaldı. `ReelDetectorRegistry`/`ReelBlockLogic` mantığı aynı; `hasExceededReelBudget` korundu.

**Engelleme ekranı butonu yeniden tasarlandı** (kullanıcı isteği): "1 saat izin ver" kaldırıldı (ve artık ulaşılamayan `reelSuppressUntilMs`/`isReelBlockSuppressed`/`REEL_SUPPRESS_DURATION_MS` + 4 testi temizlendi). Reel bloğundaki buton artık kullanıcıyı **o an açık uygulamanın kendi ana sayfasına** gönderiyor (app-limit/oturum blokları değişmedi — onlar hâlâ `GLOBAL_ACTION_HOME` ile Android launcher'a gider, çünkü tüm uygulama engelli).

**Navigasyonda üç tuzak, cihaz teşhisiyle sırayla çözüldü**:
1. `removeOverlayInternal` `currentPackage`'ı null'lıyor (app-limit için doğru: uygulamadan çıkış → yeni giriş sıfırlar). Ama app-içi Home'a geçişte paket sınırı geçilmez → null kalınca `checkReelContent` erken dönüyor, reels'e geri girince bloklanmıyordu. **Fix**: `navigateToAppHome` sonda `currentPackage`'ı geri yüklüyor.
2. Geçiş anlık değil → reel hâlâ ağaçtayken anında yeniden bloklanıp **sonsuz döngü**. **Fix**: `REEL_HOME_GRACE_MS = 2500L` grace penceresi (`reelHomeGraceUntilMs`), `checkReelContent` bu pencerede erken döner.
3. **`performAction(ACTION_CLICK)` Instagram'da no-op** — Litho sanal node'larında `true` döner ama hiçbir şey yapmaz (aynı reel app değişimine rağmen sabit kaldı). Alt-nav bounds'una `dispatchGesture` tap'i de iki app'te güvenilmez çıktı (reel sabit / yanlış video). **Fix**: `navigateToAppHome` artık sadece `performGlobalAction(GLOBAL_ACTION_BACK)` — gerçek sistem event'i, iki app'te de reels/shorts yüzeyinden Home feed'e iner. Home feed reel-oynatıcı dizesiyle eşleşmediği için döngü olmuyor. Her iki app cihazda doğrulandı.

**Sınır**: tespit cihaz dili Türkçe varsayar (uygulamanın geneli zaten Türkçe). Dil değişirse metinler değişir. `README.md` bu oturumda eklendi.

## Devam eden / ertelenmiş iş

(Şu an bekleyen/ertelenmiş iş yok. Madde 30'daki Bug A'nın kök nedeni — queryIntentActivities'in ara sıra eksik dönmesi — tam teşhis edilmedi, kısa TTL ile semptomu bastırıldı; gerçek cihazda tekrar "Kullanım yok" görülürse whitelist→blacklist geçişi düşünülmeli.)

## Doğrulama notu

Madde 30'dan itibaren Pixel_6 AVD (Android 16) üzerinde gerçek cihaz doğrulaması yapılabiliyor — bu ortamda artık emülatör var. Daha önceki maddelerdeki "cihaz doğrulaması kullanıcıda" notları hâlâ geçerli olabilir (bu turda yalnızca madde 28 ve 29 açıkça test edildi); tema geçişi/kategori renkleri gibi büyük ölçüde görsel konular ayrıca gözden geçirilmedi.
